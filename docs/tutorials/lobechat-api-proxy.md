...
LobeChat 可以连接 OpenAI-compatible API endpoint，用便宜 GPT API 承接日常聊天和测试。
...
2. 在 LobeChat 的模型供应商或 OpenAI-compatible 配置里填写 Key。
3. 把 OpenAI-compatible API endpoint 指向 https://yesapi.online/v1。
...
LobeChat can connect to an OpenAI-compatible endpoint for lower-cost GPT-style chat usage.
...
2. Paste it into the LobeChat provider settings.
3. Set OpenAI-compatible API endpoint to https://yesapi.online/v1.
