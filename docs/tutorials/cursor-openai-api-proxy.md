...
在 Cursor 里把官方 OpenAI 接口换成低成本 AI API 中转站，同时保留流式输出和常见模型调用方式。
...
2. 在 Cursor 的模型供应商或 OpenAI-compatible 配置里填写 Key。
3. 把 OpenAI-compatible Base URL 指向 https://yesapi.online/v1。
...
Use Cursor with a lower-cost OpenAI API proxy while keeping the standard OpenAI-compatible request shape.
...
2. Paste it into the Cursor provider settings.
3. Set OpenAI-compatible Base URL to https://yesapi.online/v1.
