...
很多 VSCode AI 插件都支持自定义 OpenAI Base URL，可以接入 AI API 中转站降低调用成本。
...
2. 在 VSCode AI extensions 的模型供应商或 OpenAI-compatible 配置里填写 Key。
3. 把 OpenAI Base URL 指向 https://yesapi.online/v1。
...
Many VSCode AI extensions support a custom OpenAI Base URL, which makes an API proxy gateway easy to use.
...
2. Paste it into the VSCode AI extensions provider settings.
3. Set OpenAI Base URL to https://yesapi.online/v1.
