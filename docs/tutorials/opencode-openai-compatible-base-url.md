...
OpenCode 可以使用 OpenAI-compatible API gateway，只需要把模型供应商的 Base URL 指向 YesAPI。
...
2. 在 OpenCode 的模型供应商或 OpenAI-compatible 配置里填写 Key。
...
OpenCode can use an OpenAI-compatible API gateway by changing the provider Base URL.
...
2. Paste it into the OpenCode provider settings.
