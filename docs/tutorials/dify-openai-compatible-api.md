...
Dify 工作流和应用可以接入 OpenAI-compatible API gateway，方便统一模型调用成本。
...
2. 在 Dify 的模型供应商或 OpenAI-compatible 配置里填写 Key。
3. 把 OpenAI-compatible provider endpoint 指向 https://yesapi.online/v1。
...
Dify apps and workflows can connect to an OpenAI-compatible API gateway as a custom provider endpoint.
...
2. Paste it into the Dify provider settings.
3. Set OpenAI-compatible provider endpoint to https://yesapi.online/v1.
