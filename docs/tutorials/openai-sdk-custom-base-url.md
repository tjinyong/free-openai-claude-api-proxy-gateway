...
如果项目已经使用 OpenAI SDK，接入中转站通常只需要修改 base_url 和 API Key。
...
2. 在 OpenAI SDK 的模型供应商或 OpenAI-compatible 配置里填写 Key。
3. 把 base_url / baseURL 指向 https://yesapi.online/v1。
...
Existing OpenAI SDK projects usually only need a base_url and API key change to use a third-party endpoint.
...
2. Paste it into the OpenAI SDK provider settings.
3. Set base_url / baseURL to https://yesapi.online/v1.
