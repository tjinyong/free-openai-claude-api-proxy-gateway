...
LangChain 项目通常可以通过模型客户端参数设置第三方 Base URL，继续使用 OpenAI SDK 兼容格式。
...
2. 在 LangChain 的模型供应商或 OpenAI-compatible 配置里填写 Key。
3. 把 custom base_url 指向 https://yesapi.online/v1。
...
LangChain projects can usually pass a custom base_url to the OpenAI-compatible client.
...
2. Paste it into the LangChain provider settings.
3. Set custom base_url to https://yesapi.online/v1.
