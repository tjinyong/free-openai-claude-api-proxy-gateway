...
NextChat 适合个人和团队聊天场景，通过 OpenAI API proxy 可以统一 Key 和模型入口。
...
2. 在 NextChat 的模型供应商或 OpenAI-compatible 配置里填写 Key。
3. 把 OpenAI API endpoint 指向 https://yesapi.online/v1。
...
NextChat works well with an OpenAI API proxy for a single endpoint across multiple model families.
...
2. Paste it into the NextChat provider settings.
3. Set OpenAI API endpoint to https://yesapi.online/v1.
