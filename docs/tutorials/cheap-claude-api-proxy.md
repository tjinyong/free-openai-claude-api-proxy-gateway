...
需要 Claude 模型能力但希望控制调用成本时，可以使用 Claude API proxy 或统一网关入口。
...
2. 在 Claude-compatible clients 的模型供应商或 OpenAI-compatible 配置里填写 Key。
3. 把 Claude API proxy endpoint 指向 https://yesapi.online/v1。
...
A Claude API proxy can reduce cost and centralize access for Claude-compatible workflows.
...
2. Paste it into the Claude-compatible clients provider settings.
3. Set Claude API proxy endpoint to https://yesapi.online/v1.
